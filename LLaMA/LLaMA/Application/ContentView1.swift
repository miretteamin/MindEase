...
struct ContentView1: View {
  ...
  @StateObject private var logManager = LogManager()
    @State private var isFirstGeneration = true
  ...
  @ObservedObject var mindViewModel: MindWaveViewModel
  ...
  private func generate() {
    ...
    let seq_len = 256 // text: 256, vision: 768
    ...
    let useLlama = modelPath.lowercased().contains("llama")
      ...
      if isFirstGeneration {
                  let initialMeditationPrompt = "You are currently in a meditation session. Please provide insights or guidance for a calm and focused state."
                  messages.append(Message(type: useLlama ? .llamagenerated : .llavagenerated, text: initialMeditationPrompt))
                  isFirstGeneration = false // Set the flag to false after the first use
              }
    ...
    runnerQueue.async {
      ...
      do {
        ...
        let attentionString = mindViewModel.attentionValues.map { String($0) }.joined(separator: ", ")
        let initialPrompt = "User's Attention Values: [\(attentionString)]\n\nUser: \(userPrompt)"
